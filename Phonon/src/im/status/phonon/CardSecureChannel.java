 ...
 */
public class CardSecureChannel {
...
//    idKeypair.genKeyPair();
   ...
   */
  public void CardinitSecureChannel(byte[] aPairingSecret, short off)
  {
...
//    KeyAgreement tecdh = crypto.GetKeyAgreement();
   ...
   */
  public void CardopenSecureChannel(APDU apdu) {
    preassignedPairingOffset = -1;
    mutuallyAuthenticated = false;

    byte[] apduBuffer = apdu.getBuffer();

    short pairingKeyOff = CardcheckPairingIndexAndGetOffset(apduBuffer[ISO7816.OFFSET_P1]);
   ...
   * @param apdu the JCRE-owned APDU object.
   */
  public void CardmutuallyAuthenticate(APDU apdu) {
    ...
    byte[] apduBuffer = apdu.getBuffer();
    short len = CardpreprocessAPDU(apduBuffer);
   ...
   * @param apdu the JCRE-owned APDU object.
   */
  public void CardloadCert(byte[] IncomingCert, short IncomingCertLen ) {
 //   byte[] apduBuffer = apdu.getBuffer();
    ...
    if (IncomingCertLen <= (byte)CERTIFICATE_MAX_LEN) {
      Util.arrayCopyNonAtomic(IncomingCert, (short)0, idCertificate, (short) 0, IncomingCertLen);
   ...
   * @param apdu the JCRE-owned APDU object.
   */
  public void CardidentifyCard(APDU apdu) {
    byte[] apduBuffer = apdu.getBuffer();
   ...
   * @param apdu the JCRE-owned APDU object.
   */
  public void Cardpair(APDU apdu) {
    if (CardisOpen()) {
      ISOException.throwIt(ISO7816.SW_CONDITIONS_NOT_SATISFIED);
     }
  ...
  public short CardGetCardCertificate( byte[] ReturnBuffer)
  {
  ...
  public short CardGetCardPublicKey( byte[] ReturnBuffer)
  {
/*	    ECPublicKey pk = (ECPublicKey) idKeypair.getPublic();
	    short pubkeyLen = pk.getW(ReturnBuffer, (short) (0)); // Copy pubkey after TLV type and len
	    return pubkeyLen;
*/
	  byte certType = idCertificate[0];
	  byte certLen = idCertificate[1];
	  byte permType = idCertificate[2];
	  byte permLen = idCertificate[3];
	  byte pubKeyLen = idCertificate[ 5 + permLen];
	  Util.arrayCopyNonAtomic(idCertificate, (short)( 6+ permLen), ReturnBuffer, (short)0, (short)pubKeyLen);
	  return (short)pubKeyLen;
  }
   ...
   */
  private short CardpairStep1(byte[] SenderSalt ) {
...
//    crypto.sha256.update(apduBuffer, (short) 0, SC_SECRET_LENGTH);
   ...
   * @return the length of the reply
   */
  private short CardpairStep2(byte[] apduBuffer) { 
    // Validate command data length
   ...
   * @param apduBuffer the APDU buffer
   */
  public void Cardunpair(byte[] apduBuffer) {
    short off = CardcheckPairingIndexAndGetOffset(apduBuffer[ISO7816.OFFSET_P1]);
   ...
   */
  public short CardcopyPublicKey(byte[] buf, short off) {
   ...
   */
  public byte CardgetRemainingPairingSlots() {
   ...
   */
  public void CardupdateSecureChannelCounter() {
   ...
   */
  public void CardupdatePairingSecret(byte[] aPairingSecret, byte off) {
    Util.arrayCopy(aPairingSecret, off, pairingSecret, (short) 0, SC_SECRET_LENGTH);
  }
   ...
   * @return the offset
   */
  private short CardcheckPairingIndexAndGetOffset(byte idx) 
  {
