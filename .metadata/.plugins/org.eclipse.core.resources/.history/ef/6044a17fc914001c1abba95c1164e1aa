...
//	  static final byte APPLICATION_CAPABILITIES = (byte)(CAPABILITY_SECURE_CHANNEL | CAPABILITY_KEY_MANAGEMENT | CAPABILITY_CREDENTIALS_MANAGEMENT | CAPABILITY_NDEF);
	 ...
	 static final byte INS_CARD_PAIR				= (byte) 0x51;
...
//		    cardsecp256k1 = new SECP256k1(crypto);
//		    cardsecureChannel = new CardSecureChannel(PAIRING_MAX_CLIENT_COUNT, crypto, secp256k1 ); // secureChannel.GetKeyPair());
...
if(DEBUG_MODE)
{
	if((buf[ISO7816.OFFSET_INS] != INS_CREATE_PHONON  )
				...
				&& (buf[ISO7816.OFFSET_INS] != INS_CARD_PAIR) 
...
//	    	secureChannel.respond( apdu,  apduBuffer,  Offset, ISO7816.SW_NO_ERROR);  	
	...
	private void CardPairStep1( APDU apdu )
	{
        ...
        Bertlv berCardCert = BertlvArray[0];
        byte [] IncomingPhonon = IncomingData;

        berCardCert.LoadTag(IncomingPhonon);
        if( berCardCert.GetTag() != TLV_CARD_CERTIFICATE )
        {
			secureChannel.respond( apdu, (short)0, ISO7816.SW_WRONG_DATA);
			return;
        }
        
        cardsecureChannel.CardloadCert(berCardCert.GetData(), berCardCert.GetLength());
        Bertlv berPairSalt = BertlvArray[1];
		berCardCert.LoadNextTag(berPairSalt);
		if( berPairSalt.GetTag() != TLV_SALT)
		{
			secureChannel.respond( apdu, (short)0, ISO7816.SW_WRONG_DATA);
			return;			
		}
		short cardIncomingSalt;
		Util.getShort(berPairSalt.GetData(),(short)0);
